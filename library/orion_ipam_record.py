...
def run_module():
    ...
    for i in range(module.params['retry_limit']):
        ...
        if module.params['ping_test']:
            ping_response = os.system("/usr/bin/ping -c 1 -W 3 {0}".format(results["results"][i]['IPAddress']))
            module.fail_json(msg="Ping Response is {0}".format(ping_response), **result)
            if ping_response == 0:
    ...
    if ip_address:
        ...
        # Set the Comments
        try:
            swis.update(uri, Comments="{0}".format(module.params['new_ip_comment']))
        except Exception as e:
